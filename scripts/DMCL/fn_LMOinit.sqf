...
[
	...
	{
		remoteExec ["XEPKEY_fn_diaryContent",0,true];
	},
	[]
...
[
	{
		/* if (!LMO_active) then {
			["LMO_mStart"] call CBA_fnc_serverEvent;
		}; */
		if (count (allUnits select {side _x == GRLIB_side_enemy}) > 0) then {
			...
			if (LMO_active == false && count LMO_enyList > 0 && ((LMO_mChance <= LMO_mChanceSelect) || LMO_Debug)) then {
				LMO_active = true;
